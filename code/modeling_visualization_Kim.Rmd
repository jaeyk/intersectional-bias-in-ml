...
# All predictioned classes (race, gender, and party ID) were merged into this one CSV file 

merged <- read_csv("/home/jae/bias-in-ml/twitter/processed_data/Final_Predictions.csv")[,-1]
...
#################### DV ####################
    "Type", 
#################### IV or Moderating Variables #################### 
    "Racial Demographic (All)", "Logistic Model Results", "Personal Attack?", "Political Leaning", "Number of Votes") %>%
  # Rename (intuitive new col names = old col names)
  rename(race = "Racial Demographic (All)",
         gender = "Logistic Model Results",
         personal_attack = "Personal Attack?",
         party_id = "Political Leaning",
  # Also, I prefer lower case
         type = "Type",
...
gender_plot <- df %>%
  ...
  mutate(prop = n / sum(n),
         prop = round(prop,2)) %>% 
  filter(type %in% c("Abusive", "Normal", "Hateful")) %>%
  unite(race_gender, c("race", "gender")) %>%
  mutate(race_gender = str_replace(race_gender, "_", " ")) %>%
  ggplot(aes(x = race_gender, y = prop, fill = type)) +
...
party_plot <- df %>%
  ...
  ggplot(aes(x = race_party, y = prop, fill = type)) +
    geom_col(position = "fill") +
...
df <- df %>%
  mutate(hateful = ifelse(type == "Hateful", 1, 0),
         abusive = ifelse(type == "Abusive", 1, 0),
         normal = ifelse(type == "Normal", 1, 0),
...
### 4.2. T-test with bootstapped samples 
